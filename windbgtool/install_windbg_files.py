...
(bits, os_name) = platform.architecture()
...
if bits == '64bit':
    windbg_folders64.append(os.environ["ProgramFiles"] + r'\Windows Kits\10\Debuggers\x64')
    windbg_folders64.append(os.environ["ProgramFiles"] + r'\Debugging Tools for Windows (x64)')

    windbg_folders32.append(os.environ["ProgramFiles(x86)"] + r'\Windows Kits\10\Debuggers\x86')
    windbg_folders32.append(os.environ["ProgramFiles"] + r'\Debugging Tools for Windows (x64)\Wow64')
...
for windbg_folder in windbg_folders64:
    ...
    for python_folder in python_folders64:
...
for windbg_folder in windbg_folders32:
    ...
    for python_folder in python_folders32:
        if not os.path.isdir(python_folder):
            continue
